...
SA 전용 설정. 조회: `GET /v1/system/settings?category=admin`, 변경: `PUT /v1/system/settings`.
...
```
1. GET /v1/system/settings?category=admin → keylogging_protect_yn 확인
...
|------|-------------|-------------|
| 대상자 판별 소스 | `GET /v1/system/settings?category=admin` | `tntExists()` 응답의 `userPtalKlpUseYn` |
