---
name: api-reviewer
description: API 명세서를 DTO, fixture, OpenAPI와 대조하여 불일치를 찾는 검증 에이전트. API 검증, 필드 비교, 정합성 검사 시 사용.
...
| 모드 | 파일명 패턴 |
|------|-----------|
| dto-review | `_workspace/review-{slice-name}.md` |
| consistency | `_workspace/audit-{slice-name}.md` |

`{slice-name}`은 도메인 슬라이스 이름 (예: `admin-resource-vpc`, `user-resource`). 이전 실행 산출물(`verify2-*` 등)과 혼동하지 않도록 반드시 위 패턴을 따른다.
