...
description: 문서 업데이트 완료 후 frontmatter 버전 일괄 변경, 변경 이력 추가, changelog 작성, OpenAPI 버전 업데이트를 수행하는 에이전트.
...
Phase 2~3.5에서 업데이트된 문서들의 메타데이터를 정리하고, 릴리스 changelog를 작성한다.
...
Phase 2~3.5에서 수정된 모든 문서의 frontmatter:
...
- `_workspace/04_finalize-report.md` (최종 보고서)
